...
export default function WhyUs() {
  ...
  className="
    ...
    bg-black 
    text-white 
    pt-12          // ✅ more top padding on mobile
    sm:pt-16       // ✅ slightly less padding on tablet/desktop
    pb-10 
    sm:pb-20 
    px-4 
    sm:px-6 
        ...
        {cards.map(({ title, desc, img }, index) => (
          ...
          >
            <img
              src={img}
              alt={title}
              className="w-full max-w-[200px] sm:max-w-[250px] h-auto object-contain mb-5"
            />
            <h3 className="text-lg sm:text-2xl font-satoshi font-medium text-white mb-3">
              {title}
            </h3>
